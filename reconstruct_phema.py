...
def reconstruct_phema(
    ...
    if out_nimg is None:
        ...
        raise click.ClickException(
            "Reconstruction time must match one of the input pickles"
        )
    ...
    if skip_existing and out_dir is not None:
        out_std = [
            std
            for std in out_std
            if not os.path.isfile(pkl_path(out_dir, out_prefix, out_nimg, std))
        ]
    num_batches = (len(out_std) - 1) // max_batch_size + 1
    out_std_batches = np.array_split(out_std, num_batches)
    if verbose:
        print(
            f"Reconstructing {len(out_std)} output pickles in {num_batches} batches..."
        )
        for i, batch in enumerate(out_std_batches):
            for std in batch:
                print(f"    batch {i}: ", end="")
                print(
                    pkl_path(out_dir, out_prefix, out_nimg, std)
                    if out_dir is not None
                    else pkl_path("", "<yield>", out_nimg, std)
                )
    ...
    class ReconstructionIterable:
        ...
        def __iter__(self):
            ...
            for out_std_batch in out_std_batches:
                coefs = training.phema.solve_posthoc_coefficients(
                    in_nimg, in_std, out_nimg, out_std_batch
                )
                out = [
                    dnnlib.EasyDict(net=None, nimg=out_nimg, std=std)
                    for std in out_std_batch
                ]
                r.out = []
                ...
                for i in range(len(in_pkls)):
                    ...
                    if i == len(in_pkls) - 1:
                        for j in range(len(out)):
                            ...
                            out[j].pkl_path = pkl_path(
                                out_dir, out_prefix, out_nimg, out[j].std
                            )
...
def parse_std_list(s):
    ...
    for i, v in enumerate(raw):
        ...
        if abs(num - approx_num) > 1e-4:
            raise click.ClickException(
                "'...' must correspond to an evenly spaced interval"
            )
        for j in range(num):
    ...
    if not all(0.000 < v < 0.289 for v in out):
        raise click.ClickException(
            "Relative standard deviation must be positive and less than 0.289"
        )
    return out
...
@click.option(
    "--indir",
    "in_dir",
    help="Directory containing the input pickles",
    metavar="DIR",
    type=str,
    required=True,
)
@click.option(
    "--inprefix",
    "in_prefix",
    help="Filter inputs based on filename prefix",
    metavar="STR",
    type=str,
    default=None,
)
@click.option(
    "--instd",
    "in_std",
    help="Filter inputs based on standard deviations",
    metavar="LIST",
    type=parse_std_list,
    default=None,
)
@click.option(
    "--outdir",
    "out_dir",
    help="Where to save the reconstructed network pickles",
    metavar="DIR",
    type=str,
    required=True,
)
...
@click.option(
    "--outstd",
    "out_std",
    help="List of desired relative standard deviations",
    metavar="LIST",
    type=parse_std_list,
    required=True,
)
@click.option(
    "--outkimg",
    "out_kimg",
    help="Training time of the snapshot to reconstruct",
    metavar="KIMG",
    type=click.IntRange(min=1),
    default=None,
)
@click.option(
    "--skip", "skip_existing", help="Skip output files that already exist", is_flag=True
)
@click.option(
    "--batch",
    "max_batch_size",
    help="Maximum simultaneous reconstructions",
    metavar="INT",
    type=click.IntRange(min=1),
    default=8,
    show_default=True,
