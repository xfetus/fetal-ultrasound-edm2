...
def training_loop(
    ...
    dist.print0(f'Training from {state.cur_nimg // 1000} kimg to {stop_at_nimg // 1000} kimg:', flush=True)
    dist.print0()
    ...
    if val_kwargs is not None and dist.get_rank() == 0:
        ...
        dist.print0(f'Validation: {N_val} images | '
        ...
        if status_nimg is not None and (done or state.cur_nimg % status_nimg == 0) and (state.cur_nimg != start_nimg or start_nimg == 0):
            ...
            if val_images_fixed is not None:
                val_loss = _compute_val_loss(
                    net, val_images_fixed, val_labels_fixed,
                    val_sigma_fixed, val_noise_fixed,
                    loss_fn.sigma_data, batch_gpu,
                )
                dist.print0(f'Val_loss:  {training_stats.report0("Loss/val_loss", val_loss):.4f}', flush=True)
