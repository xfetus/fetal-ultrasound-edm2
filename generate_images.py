...
def generate_images(
    ...
    **sampler_kwargs,                           # Additional arguments for the sampler function.
    ...
    network_kwargs = {'class_name': 'training.networks_edm2.Precond', 'model_channels': 192, 'dropout': 0.0, 'use_fp16': True} 
    ...
    ckpt = torch.load("/scratch/hgm1g14/SpanishDataset/output_dir_s_with_flip/training-state-0001310.pt", map_location='cpu', weights_only=False)
    #print(ckpt['ema']['emas'])
    ...
    network_kwargs = {'class_name': 'training.networks_edm2.Precond', 'model_channels': 384, 'dropout': 0.10, 'use_fp16': True} 
    gnet = dnnlib.util.construct_class_by_name(**network_kwargs, **interface_kwargs).to(device)
    ckpt = torch.load("/scratch/hgm1g14/SpanishDataset/output_dir_s_with_flip/training-state-0008519.pt", map_location='cpu', weights_only=False)
    #print(ckpt['ema']['emas'])
