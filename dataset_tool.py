...
def make_transform(
    ...
    if transform == "center-crop-dhariwal":
        ...
        if output_width != output_height:
            raise click.ClickException(
                "width and height must match in --resolution=WxH when using "
                + transform
                + " transform"
            )
...
@click.option(
    "--max-images", help="Maximum number of images to output", metavar="INT", type=int
)
@click.option(
    "--transform",
...
def convert(
    ...
    transform_image = make_transform(
        transform, *resolution if resolution is not None else (None, None)
    )
    dataset_attrs = None
