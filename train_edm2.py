...
config_presets = {
    "edm2-img512-xxs": dnnlib.EasyDict(
        duration=2048 << 20,
        batch=2048,
        channels=64,
        lr=0.0170,
    ...
    "edm2-img512-xs": dnnlib.EasyDict(
        duration=2048 << 20,
        batch=2048,
        channels=128,
        lr=0.0120,
    ...
    "edm2-img512-xl": dnnlib.EasyDict(
        duration=1280 << 20,
        batch=2048,
        channels=384,
        lr=0.0070,
    ...
    "edm2-img512-xxl": dnnlib.EasyDict(
        duration=896 << 20,
        batch=2048,
        channels=448,
        lr=0.0065,
...
def setup_training_config(preset="edm2-img512-s", **opts):
    ...
    c.dataset_kwargs = dnnlib.EasyDict(
        ...
        fpus23_path=opts.get("fpus23", None),
        african_path=opts.get("african", None),
        fetal_abdomen_path=opts.get("fetal_abdomen", None),
    )
    # Validation uses Spanish dataset only — no auxiliary datasets.
    c.val_kwargs = dnnlib.EasyDict(
        class_name="training.dataset.UltrasoundDataset",
        path=opts.data,
        use_labels=opts.get("cond", True),
        split="val",
    ...
    try:
        ...
        if c.dataset_kwargs.use_labels and not dataset_obj.has_labels:
            raise click.ClickException(
                "--cond=True, but no labels found in the dataset"
            )
        c.val_kwargs.num_classes = (
            dataset_obj.label_dim
        )  # ensure val labels match training label_dim
    ...
    if dataset_channels == 3:
        c.encoder_kwargs = dnnlib.EasyDict(
        ...
        raise click.ClickException(
            f"--data: Unsupported channel count {dataset_channels}"
        )
...
@click.option(
    "--fpus23",
    help="Path to FPUS23 dataset (optional)",
    metavar="DIR",
    type=str,
    default=None,
)
@click.option(
    "--african",
    help="Path to African Zenodo dataset root (optional)",
    metavar="DIR",
    type=str,
    default=None,
)
@click.option(
    "--fetal-abdomen",
    help="Path to fetal abdominal segmentation IMAGES dir (optional)",
    metavar="DIR",
    type=str,
    default=None,
)
...
@click.option(
    "--batch", help="Total batch size", metavar="NIMG", type=parse_nimg, default=None
)
@click.option(
    "--channels",
...
@click.option(
    "--lr",
    help="Learning rate max. (alpha_ref)",
    metavar="FLOAT",
    type=click.FloatRange(min=0, min_open=True),
    default=None,
)
@click.option(
    "--decay",
    help="Learning rate decay (t_ref)",
    metavar="BATCHES",
...
@click.option(
    "--status",
    help="Interval of status prints",
    metavar="NIMG",
    type=parse_nimg,
    default="128Ki",
    show_default=True,
...
@click.option(
    "--snapshot",
    help="Interval of network snapshots",
    metavar="NIMG",
    type=parse_nimg,
    default="8Mi",
    show_default=True,
...
@click.option(
    "--checkpoint",
    help="Interval of training checkpoints",
    metavar="NIMG",
    type=parse_nimg,
    default="128Mi",
    show_default=True,
...
@click.option(
    "--seed", help="Random seed", metavar="INT", type=int, default=0, show_default=True
)
