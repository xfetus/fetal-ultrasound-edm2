...
class ToyModel(torch.nn.Module):
    ...
    def forward(self, x, sigma=0):
        sigma = (
            torch.as_tensor(sigma, dtype=torch.float32, device=x.device)
            .broadcast_to(x.shape[:-1])
            .unsqueeze(-1)
        )
        x = x / (self.sigma_data**2 + sigma**2).sqrt()
        y = self.layers(torch.cat([x, sigma.log() / 4, torch.ones_like(sigma)], dim=-1))
        z = (y**2).mean(-1) * self.gain / sigma.squeeze(-1) - 0.5 * (x**2).sum(
            -1
        )  # preconditioning
        return z
    ...
    def score(self, x, sigma=0, graph=False):
        ...
        score = torch.autograd.grad(
            outputs=[logp.sum()], inputs=[x], create_graph=graph
        )[0]
        return score
...
def do_train(
    ...
    # Initialize model.
    net = (
        ToyModel(num_layers=num_layers, hidden_dim=hidden_dim, sigma_data=sigma_data)
        .to(device)
        .train()
        .requires_grad_(True)
    )
    ...
    for iter_idx in tqdm.tqdm(range(total_iter)):
        ...
        beta = training.phema.power_function_beta(
            std=ema_std, t_next=iter_idx + 1, t_delta=1
        )
...
def do_plot(
    ...
    guidance=1,
    gnet=None,
    elems={"gt_uncond", "gt_outline", "samples"},
    view_x=0,
    view_y=0,
    view_size=1.6,
    grid_resolution=400,
    ...
    if any(
        x.startswith(y) for x in elems for y in ["samples", "trajectories", "scores"]
    ):
        samples = gt("A", device).sample(
            num_samples, sigma_max, generator=torch.Generator(device).manual_seed(seed)
        )
        if sample_distance > 0:
            ok = torch.ones(len(samples), dtype=torch.bool, device=device)
            for i in range(1, len(samples)):
                ok[i] = (samples[i] - samples[:i][ok[:i]]).square().sum(
                    -1
                ).sqrt().min() >= sample_distance
            samples = samples[ok]

    # Run sampler.
    if any(x.startswith(y) for x in elems for y in ["samples", "trajectories"]):
        trajectories = do_sample(
            net=(net or gt("A", device)),
            x_init=samples,
            guidance=guidance,
            gnet=gnet,
            sigma_max=sigma_max,
    ...
    # Initialize plot.
    gridx = torch.linspace(
        view_x - view_size, view_x + view_size, steps=grid_resolution, device=device
    )
    gridy = torch.linspace(
        view_y - view_size, view_y + view_size, steps=grid_resolution, device=device
    )
    ...
    if "p_net" in elems:
        contours(
            net.logp(gridxy, sigma_max),
            levels=np.linspace(-2.5, 2.5, num=20)[1:-1],
            cmap="Greens",
            linealpha=0.2,
        )
    if "p_gnet" in elems:
        contours(
            gnet.logp(gridxy, sigma_max),
            levels=np.linspace(-2.5, 3.5, num=20)[1:-1],
            cmap="Reds",
            linealpha=0.2,
        )
    if "p_ratio" in elems:
        contours(
            net.logp(gridxy, sigma_max) - gnet.logp(gridxy, sigma_max),
            levels=np.linspace(-2.2, 1.0, num=20)[1:-1],
            cmap="Blues",
            linealpha=0.2,
        )
    if "gt_uncond" in elems:
        contours(
            gt("AB", device).logp(gridxy),
            levels=[-2.12, 0],
            colors=[[0.9, 0.9, 0.9]],
            linecolors=[[0.7, 0.7, 0.7]],
            linewidth=1.5,
        )
    if "gt_outline" in elems:
        contours(
            gt("A", device).logp(gridxy),
            levels=[-2.12, 0],
            colors=[[1.0, 0.8, 0.6]],
            linecolors=[[0.8, 0.6, 0.5]],
            linewidth=1.5,
        )
    if "gt_smax" in elems:
        contours(
            gt("A", device).logp(gridxy, sigma_max),
            levels=[-1.41, 0],
            colors=["C1"],
            alpha=0.2,
            linealpha=0.2,
        )
    if "gt_shaded" in elems:
        contours(
            gt("A", device).logp(gridxy),
            levels=np.linspace(-2.5, 3.07, num=15)[1:-1],
            cmap="Oranges",
            linealpha=0.2,
        )
    if "trajectories" in elems:
        lines(trajectories.transpose(0, 1), alpha=0.3)
    if "scores_net" in elems:
        arrows(samples, net.score(samples, sigma_max), color="C2")
    if "scores_gnet" in elems:
        arrows(samples, gnet.score(samples, sigma_max), color="C3")
    if "scores_ratio" in elems:
        arrows(
            samples,
            net.score(samples, sigma_max) - gnet.score(samples, sigma_max),
            color="C0",
        )
    if "samples" in elems:
        points(trajectories[-1], size=15, alpha=0.25)
    if "samples_before" in elems:
        points(samples)
    if "samples_after" in elems:
...
@cmdline.command()
@click.option(
    "--outdir", help="Output directory", metavar="DIR", type=str, default=None
)
@click.option(
    "--cls",
    help="Target classes",
...
@click.option(
    "--net",
    help="Main model  [default: download]",
    metavar="PKL|URL",
    type=str,
    default="https://nvlabs-fi-cdn.nvidia.com/edm2/toy-example/clsA-layers04-dim64/iter4096.pkl",
)
@click.option(
    "--gnet",
    help="Guiding model  [default: autoguidance]",
    metavar="PKL|URL",
    type=str,
    default="https://nvlabs-fi-cdn.nvidia.com/edm2/toy-example/clsA-layers04-dim32/iter0512.pkl",
)
@click.option(
    "--guidance",
...
@click.option(
    "--save",
    help="Save figure, do not display",
    metavar="PNG|PDF",
    type=str,
    default=None,
)
def plot(net, gnet, guidance, save, device=torch.device("cuda")):
    ...
    fig1_kwargs = dict(
        view_x=0.30, view_y=0.30, view_size=1.2, num_samples=1 << 14, device=device
    )
    fig2_kwargs = dict(
        view_x=0.45,
        view_y=1.22,
    ...
    do_plot(
        net=net, elems={"gt_shaded", "trajectories", "samples_after"}, **fig2_kwargs
    )
    ...
    do_plot(
        net=net,
        elems={"p_net", "gt_smax", "scores_net", "samples_before"},
        **fig2_kwargs,
    )
    plt.subplot(2, 4, 6)
    ...
    do_plot(
        net=net,
        gnet=gnet,
        elems={"p_gnet", "gt_smax", "scores_gnet", "samples_before"},
        **fig2_kwargs,
    )
    plt.subplot(2, 4, 7)
    ...
    do_plot(
        net=net,
        gnet=gnet,
        elems={"p_ratio", "gt_smax", "scores_ratio", "samples_before"},
        **fig2_kwargs,
    )
    plt.subplot(2, 4, 8)
    ...
    do_plot(
        ...
        gnet=gnet,
        guidance=guidance,
        elems={"gt_shaded", "trajectories", "samples_after"},
        **fig2_kwargs,
    )
