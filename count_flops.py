...
def hook_torch_ops():
    ...
    def conv2d_hook(
        ...
        dilation = (
            tuple(dilation) if isinstance(dilation, (list, tuple)) else (dilation,)
        )
        return conv2d_orig(input, weight, bias, stride, padding, dilation, groups)

    def conv_transpose2d_hook(
        input,
        weight,
        bias=None,
        stride=1,
    ...
    def scaled_dot_product_attention_hook(
...
def count_flops(
    ...
    if net_args is None:
        x = torch.zeros(
            [1, net.img_channels, net.img_resolution, net.img_resolution], device=device
        )
    ...
    with hook_torch_ops():
        warnings.filterwarnings(
            "ignore",
            "Converting a tensor to a Python integer might cause the trace to be incorrect.",
        )
        warnings.filterwarnings(
            "ignore",
    ...
    for node in nodes:
        ...
        for arg in list(node.outputs()) + list(node.inputs()):
            if m := re.search(r":\s*(Float|Double|Half|Long)\(([\d\s,]*)", str(arg)):
                ...
                r":\s*(float|int|bool|str|NoneType|int\[\]|Tensor\[\]|Device|Scalar|__torch__\.torch\.classes\.profiler\._RecordFunction)\s*=",
                str(arg),
            ):
                args.append([])
            else:
...
def print_flops(res):
    ...
    print(
        f'{"Total":<26s}{sum(res.op_count.values()):<7d}{sum(res.op_flops.values())/1e9:.2f}'
    )
    print()
