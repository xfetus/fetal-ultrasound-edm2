...
metric_specs = {
    "fid": dnnlib.EasyDict(
        detector_kwargs=dnnlib.EasyDict(class_name=InceptionV3Detector)
    ),
    "fd_dinov2": dnnlib.EasyDict(
        detector_kwargs=dnnlib.EasyDict(class_name=DINOv2Detector)
    ),
}
...
def get_detector(metric, verbose=True):
    ...
    if verbose:
        name = (
            kwargs.class_name.split(".")[-1]
            if isinstance(kwargs.class_name, str)
            else kwargs.class_name.__name__
        )
...
def calculate_stats_for_iterable(
    ...
    class StatsIterable:
        ...
        def __iter__(self):
            state = [
                dnnlib.EasyDict(metric=metric, detector=detector)
                for metric, detector in zip(metrics, detectors)
            ]
            for s in state:
                s.cum_mu = torch.zeros(
                    [s.detector.feature_dim], dtype=torch.float64, device=device
                )
                s.cum_sigma = torch.zeros(
                    [s.detector.feature_dim, s.detector.feature_dim],
                    dtype=torch.float64,
                    device=device,
                )
            ...
            for batch_idx, images in enumerate(image_iter):
                if isinstance(images, dict) and "images" in images:  # dict(images)
                    ...
                    isinstance(images, (tuple, list)) and len(images) == 2
                ):  # (images, labels)
                    images = images[0]
                ...
                if batch_idx == num_batches - 1:
                    ...
                    for s in state:
                        ...
                        sigma = (
                            all_reduce(s.cum_sigma) - mu.ger(mu) * r.num_images
                        ) / (r.num_images - 1)
...
def calculate_stats_for_files(
    ...
    **stats_kwargs,  # Arguments for calculate_stats_for_iterable().
    ...
    dataset_obj = dataset.ImageFolderDataset(
        path=image_path, max_size=num_images, random_seed=seed
    )
    if num_images is not None and len(dataset_obj) < num_images:
        raise click.ClickException(
            f"Found {len(dataset_obj)} images, but expected at least {num_images}"
        )
    ...
    num_batches = (
        max((len(dataset_obj) - 1) // (max_batch_size * dist.get_world_size()) + 1, 1)
        * dist.get_world_size()
    )
    rank_batches = np.array_split(np.arange(len(dataset_obj)), num_batches)[
    ...
    return calculate_stats_for_iterable(
        image_iter=data_loader, verbose=verbose, **stats_kwargs
    )
...
def calculate_metrics_from_stats(
    ...
    for metric in metrics:
        ...
        s, _ = scipy.linalg.sqrtm(
            np.dot(stats[metric]["sigma"], ref[metric]["sigma"]), disp=False
        )
        value = float(
            np.real(m + np.trace(stats[metric]["sigma"] + ref[metric]["sigma"] - s * 2))
        )
...
@cmdline.command()
@click.option(
    "--net", help="Network pickle filename", metavar="PATH|URL", type=str, required=True
)
@click.option(
    "--ref",
    "ref_path",
...
@click.option(
    "--seed",
    help="Random seed for the first image",
    metavar="INT",
    type=int,
    default=0,
    show_default=True,
...
def gen(net, ref_path, metrics, num_images, seed, **opts):
    ...
    image_iter = generate_images.generate_images(
        net=net, seeds=range(seed, seed + num_images), **opts
    )
