...
class FetalPlaneDataset(Dataset):
    ...
    def __init__(
        self, root_dir, csv_file, transform=None, split="train", plane_filter=None
    ):
        """
        Args:
        ...
        if plane_filter is not None:
            plane_name = [
                k for k, v in self.plane_classes.items() if v == plane_filter
            ][0]
        ...
        self.operator_classes = {"Other": 0, "Op. 1": 1, "Op. 3": 2, "Op. 2": 3}
...
def main():
    parser = argparse.ArgumentParser(
        description="Calculate FID for fetal plane dataset"
    )
    parser.add_argument(
        "--real_root", type=str, required=True, help="Root directory of real images"
    )
    parser.add_argument(
        "--csv_file", type=str, required=True, help="Path to CSV file with annotations"
    )
    parser.add_argument(
        "--fake_root",
        type=str,
    ...
    parser.add_argument(
        "--tian",
        action="store_true",
        help="Use Tian et al. class index mapping for fake image directories",
    )

    args = parser.parse_args()

    # Tian et al. cond index for each of our class IDs (Other=0..Fetal thorax=5)
    tian_cond = {0: 5, 1: 4, 2: 0, 3: 1, 4: 2, 5: 3}
    ...
    fid_transform = transforms.Compose(
        [
            ...
            transforms.CenterCrop(
                128
            ),  # Center crop to 512x512 (if you want center crop first)
    ...
    for class_id, class_name in plane_classes.items():
        cond_id = tian_cond[class_id] if args.tian else class_id
        fake_dir = os.path.join(
            args.fake_root, f"diffusion_samples_FETAL_cond_{cond_id}"
        )

        if not os.path.exists(fake_dir):
            print(
                f"Warning: Directory {fake_dir} does not exist. Skipping class {class_name}."
            )
            continue
        ...
        fake_dataset = GeneratedImageDataset(root_dir=fake_dir, transform=fid_transform)
